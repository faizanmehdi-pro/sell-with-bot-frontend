...
export const AgencyAuthProvider = ({ children }) => {
  ...
  useEffect(() => {
    const token = sessionStorage.getItem("agencyAuthToken"); // ✅ Use separate token key
    setIsAuthenticated(!!token);
  }, []);

  const login = (token) => {
    sessionStorage.setItem("agencyAuthToken", token); // ✅ Use separate token key
    setIsAuthenticated(true);
  };

  const logout = () => {
    sessionStorage.removeItem("agencyAuthToken"); // ✅ Use separate token key
