...
import { verifyOtpAgencyCode } from "../../../apis/Agency/Auth/verifyOtpAgencyCode";
...
const VerifyOTPAgency = () => {
  ...
  const mutation = useMutation({
    mutationFn: ({ email, otp }) => verifyOtpAgencyCode({ email, otp }),
    onSuccess: () => {
      toast.success(
        "Your Agency Account Registered Successfully. Please Login!"
      );
      navigate("/agency");
    },
    onError: (error) => {
          ...
          {({ values, errors, touched, setFieldValue, handleBlur }) => (
            <StyledForm>
              ...
              {typeof errors.otp === "string" && (
                <ErrorText style={{ textAlign: "center", marginTop: "-10px" }}>
                  {errors.otp}
                </ErrorText>
              )}
...
export default VerifyOTPAgency;
